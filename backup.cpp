...
void setup()
{
  ...
  game.gridCount.x = int(game.mapBorder.bottomRight.x / player.OneMoveDistance);
  game.gridCount.y = int(game.mapBorder.bottomRight.y / player.OneMoveDistance);

  printNewLine("[  ok  ] Game Grid Size  Initialize.");
  ...
  printNewLine("[  ok  ] Item Parameter Initialize.");

  randomSeed(millis());

  printNewLine("[  ok  ] Random Parameter Initialize.");
...
void loop()
{
  ...
  case playGame:
  {
    ...
    if (!item.isSpawn)
    {
      // Random Location Of Item
      item.center.x = int(random(game.mapBorder.topLeft.x + 3, game.gridCount.x - 3));
      item.center.y = int(random(game.mapBorder.topLeft.y + 3, game.gridCount.y - 3));

      // Drawn To Center Of Grid
      item.center.x = (item.center.x * player.OneMoveDistance) - player.radius;
      item.center.y = (item.center.y * player.OneMoveDistance) - player.radius;

      // Drawn item
      tft.fillCircle(item.center.x, item.center.y, item.radius, item.color);

      // Set IsSpawn To True
      item.isSpawn = !item.isSpawn;
    }
    //Delete Tail Postion
    ...
    // Check Collision
    int testX = item.center.x;
    int testY = item.center.y;
    int cx = item.center.x;
    int cy = item.center.y;
    int r = item.radius;
    int rx = player.center.x;
    int ry = player.center.y;
    int rw = player.OneMoveDistance;
    int rh = player.OneMoveDistance;
    ...
    //Debug Zone
    printNewLine("[ " + mode2Str(game.mode) + " ] Move Direction : " +
                 String(status2Code(player.moveDirection)) +
                 " | Player Center   X : " +
                 String(player.center.x) +
                 " Y : " + String(player.center.y) +
                 " | Item Center  X : " + String(item.center.x) +
                 " Y : " + String(item.center.y)+
                 " | Hit Item : " +String(player.isHit));
    break;
  }
  case gameOver:
  {
